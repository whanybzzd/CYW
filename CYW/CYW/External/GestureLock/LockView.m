...
{
    if (_lineColor == nil) {
        _lineColor = [UIColor colorWithHexString:@"#f52735"];
    }
    return _lineColor;
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    NSLog(@"绘制的密码:%@",password);
    if (password.length<6) {
        
        if ([self.delegate respondsToSelector:@selector(setPassWordFailer:)]) {
            [self.delegate setPassWordFailer:@"密码长度不能小于6位"];
        }
        [self clear];
    }else{
        
        NSString *guesture=[[NSUserDefaults standardUserDefaults] objectForKey:@"guesture"];
        if (1==[guesture integerValue]) {
            //登录方式
            [self clear];
            ...
            //获取本地的密码
            NSString *localpassone = [[StorageManager sharedInstance] userConfigValueForKey:kCachedUserGuestPasswordOne];
            NSString *localpasstwo = [[StorageManager sharedInstance] userConfigValueForKey:kCachedUserGuestPasswordTwo];
            if ([NSString isEmpty:localpassone]||[NSString isEmpty:localpasstwo]) {
                if ([NSString isEmpty:localpassone]) {
                    
                    //保存第一次绘制的密码
                    [[StorageManager sharedInstance] setUserConfigValue:password forKey:kCachedUserGuestPasswordOne];
                    NSLog(@"保存第一次密码");
                    ...
                    //保存第二次绘制的密码
                    [[StorageManager sharedInstance] setUserConfigValue:password forKey:kCachedUserGuestPasswordTwo];
                    NSLog(@"保存第二次密码");
                                    if ([self.delegate respondsToSelector:@selector(setPassWordSuccess:)]) {
...
- (void)clear
{
    ...
    self.lineColor = [UIColor colorWithHexString:@"#f52735"];
