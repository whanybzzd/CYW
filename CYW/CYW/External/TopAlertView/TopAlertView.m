...
- (id)init{
  ...
  if (self = [super init]){
      ...
      self.frame = CGRectMake(0, kDevice_Is_iPhoneX?88:64, SCREEN_WIDTH, kAlertViewHeight);
...
- (void)setupView:(UIColor *)color andStayTime:(CGFloat)setStayTime andImageName:(NSString *)imageStr andTitleStr:(NSString *)titleStr andTitleColor:(UIColor *)titleColor
{
    ...
    NSString *str=[NSString stringWithFormat:@"总计:%@元",titleStr];
    [self.titleLab setAttributedText:[NSMutableAttributedString withTitleString:str RangeString:titleStr ormoreString:nil color:[UIColor colorWithHexString:@"#f52735"]]];
...
- (void)endPosition
{
    self.center = CGPointMake(SCREEN_WIDTH/2, kDevice_Is_iPhoneX?88+20:64+20);
}
