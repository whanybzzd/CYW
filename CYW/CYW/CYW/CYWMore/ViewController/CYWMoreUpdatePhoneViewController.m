//
//  CYWMoreUpdatePhoneViewController.m
//  CYW
//
...
#import "CYWMoreUpdatePhoneViewController.h"
#import "CYWMoreUpdatePhoneViewModel.h"
@interface CYWMoreUpdatePhoneViewController ()<UITextFieldDelegate>
...
@implementation CYWMoreUpdatePhoneViewController
...
- (void)initSubView{
    ...
    self.updateViewModel.phone=self.params[@"phone"];
    ...
    [[self.codeButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        @strongify(self);
        [self hideKeyboard];
        self.updateViewModel.type=@"update_phone1";
    ...
    [[self.submitButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        [self hideKeyboard];
        self.updateViewModel.op=@"1";
        [self showHUDLoading:nil];
        
        [[self.updateViewModel.refreshsubmitCommand execute:nil] subscribeNext:^(id  _Nullable x) {
            
            [self hideKeyboard];
            [self pushViewController:@"CYWMoreUpdatePhoneNextViewController"];
        } error:^(NSError * _Nullable error) {
...
- (UITextField *)newpasswordTextField{
    ...
    if (!_newpasswordTextField) {
        ...
        _newpasswordTextField.text=self.params[@"phone"];
