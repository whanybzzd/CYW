//
//  CYWAssetsPlanTwoTableViewCell.m
//  CYW
//
...
#import "CYWAssetsPlanTwoTableViewCell.h"
@interface CYWAssetsPlanTwoTableViewCell()
...
@end
@implementation CYWAssetsPlanTwoTableViewCell
...
- (void)initSubView{
    ...
    [RACObserve(self, model) subscribeNext:^(InvestViewModel *viewModel) {
        ...
        [self.repaymentsButton setTitle:[NSString stringWithFormat:@"第%@期",mapModel.unPaidPeriod] forState:UIControlStateNormal];
        if([viewModel.status isEqualToString:@"complete"]){
        ...
        }else if ([viewModel.status isEqualToString:@"overdue"]){//逾期
        ...
        self.repaymentslabel.text=[NSString stringWithFormat:@"还款日:%@",mapModel.nextRepayDate];
        [self.repaymentslabel sizeToFit];
        
        //self.timelabel.text=[NSString stringWithFormat:@"利息:%.2lf元",[mapModel.interest floatValue]];
